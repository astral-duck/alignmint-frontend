  ...
  const handleMultiNonprofitExport = (selectedNonprofitIds: string[], format: 'pdf' | 'xlsx') => {
    ...
    selectedNonprofitIds.forEach(nonprofitId => {
      ...
      if (format === 'xlsx') {
        ...
        const filename = `Cash_Flow_Statement_${nonprofit.name.replace(/\s+/g, '_')}_${dateRange.startDate}_to_${dateRange.endDate}`;
        exportToExcel(data, headers, filename, 'Cash Flow Statement');
      } else {
        const data = prepareProfitLossForExport(profitLossData);
        const headers = ['Category', 'Amount'];
        ...
        <CardContent className="space-y-6">
          {profitLoss ? (
            <>
              ...
              <div>
                <h3 className="mb-4">Cash Flows from Operating Activities</h3>
              ...
              <div>
                <h3 className="mb-4">Cash Disbursements for Operations</h3>
              ...
              <div className="space-y-3">
                <div className={`flex justify-between p-4 rounded-lg font-semibold ${
